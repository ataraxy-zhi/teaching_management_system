...
import userframe.AdministratorFrame;
import userframe.StudentFrame;
import userframe.TeacherFrame;
...
public class Login extends JFrame{
	...
	public Login() {
		...
		/*
		 * JLabel labelIdentity = new JLabel("请选择身份"); labelIdentity.setBounds(15, 102,
		 * 94, 21); contentPane.add(labelIdentity);
		 */
		
		/*
		 * rbtnAdmin = new JRadioButton("管理员"); rbtnAdmin.setBounds(141, 98, 94, 29);
		 * contentPane.add(rbtnAdmin);
		 */
		
		/*
		 * rbtnTeac = new JRadioButton("教师"); rbtnTeac.setBounds(283, 98, 94, 29);
		 * contentPane.add(rbtnTeac);
		 */
		
		/*
		 * rbtnStu = new JRadioButton("学生"); rbtnStu.setBounds(418, 98, 94, 29);
		 * contentPane.add(rbtnStu);
		 */
		
		/*
		 * //设定单选按钮集合 ButtonGroup bg = new ButtonGroup(); bg.add(rbtnAdmin);
		 * bg.add(rbtnTeac); bg.add(rbtnStu); rbtnStu.setSelected(true);//默认学生身份被选中
		 */		
		JLabel labelAccount = new JLabel("账号");
		labelAccount.setBounds(15, 127, 56, 21);
		contentPane.add(labelAccount);
		
		tfAccount = new JTextField();
		tfAccount.setBounds(139, 124, 305, 27);
		contentPane.add(tfAccount);
		tfAccount.setColumns(10);
		
		JLabel labelPassword = new JLabel("密码");
		labelPassword.setBounds(15, 218, 56, 21);
		contentPane.add(labelPassword);
		
		pfPassword = new JPasswordField();
		pfPassword.setBounds(141, 214, 303, 27);
		contentPane.add(pfPassword);
		
		JButton btnLogin = new JButton("登录");
		btnLogin.setBounds(81, 394, 79, 29);
		...
		btnLogout.setBounds(616, 394, 94, 29);
		contentPane.add(btnLogout);
		
		  btnLogout.addActionListener(new ActionListener() {
		  
		  @Override public void actionPerformed(ActionEvent e) { btnLogoutClicked(e); }
		  });
		...
		try {
			...
			if(id.equals("") || String.valueOf(pfPassword.getPassword()).equals("")) {
				...
				try {
					...
					/*
					 * if(rbtnAdmin.isSelected()) { role="administrator"; }
					 * if(rbtnTeac.isSelected()) { role="teacher"; } if(rbtnStu.isSelected()) {
					 * role="student"; }
					 */
					
					/*
					 * if(user.getUserRole().equals(role)) { if(role.equals("administrator")) {
					 * Administrator administrator =new Administrator(id,name,user);
					 * administrator.setVisible(true); } else if(role.equals("teacher")) { Teacher
					 * teacher=new Teacher(id, name, user); teacher.setVisible(true); } else {
					 * Student student=new Student(id, name, user); student.setVisible(true); }
					 * 
					 * JOptionPane.showMessageDialog(null, "登录成功！", "提示",
					 * JOptionPane.INFORMATION_MESSAGE); } else {
					 * JOptionPane.showMessageDialog(null, "输入的账号与选择的身份不符，请重新输入账号或选择身份", "提示",
					 * JOptionPane.WARNING_MESSAGE); pfPassword.setText("");
					 * //rbtnStu.setSelected(true); }
					 */
					
					
					if(user.getUserRole().equals("administrator")) {
						AdministratorFrame administrator =new AdministratorFrame(id,name,user);
						administrator.setVisible(true);
					}else if(user.getUserRole().equals("teacher")) {
						TeacherFrame teacher=new TeacherFrame(id, name, user);
						teacher.setVisible(true);
					}else if(user.getUserRole().equals("student")) {
						StudentFrame student=new StudentFrame(id, name, user);
